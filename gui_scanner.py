...
class Database:
    """Database management class for scan history (thread-safe)"""
    def __init__(self):
        self.conn = sqlite3.connect('scanner_history.db', check_same_thread=False)
...
class ScanEngine:
    """Core scanning engine with progress callback"""
    SECURITY_HEADERS = {
    ...
    def scan(self, target_url):
        ...
        self.update_progress(5, "Connecting...")
        ...
        self.update_progress(20, "Checking security headers...")
        findings.extend(self.check_security_headers(response))

        self.update_progress(35, "Checking information disclosure...")
        findings.extend(self.check_information_disclosure(response))

        self.update_progress(50, "Extracting emails...")
        findings.extend(self.extract_emails(response))

        self.update_progress(65, "Checking sensitive directories...")
        findings.extend(self.check_directories(target_url))

        self.update_progress(80, "Checking sensitive files...")
            ...
            try:
                ...
                if resp.status_code == 200 and len(resp.text) > 0:
                    findings.append({
                        ...
                        'remediation': 'Remove file from web root'
                    })
            except:
                pass
        return findings
...
class UltimateScannerGUI:
    """Main GUI application with multiple tabs"""
    ...
    def create_widgets(self):
        ...
        tk.Label(header, text="ULTIMATE VULNERABILITY ASSESSMENT FRAMEWORK",
                font=("Arial", 18, "bold"), bg='#0f3460', fg='white').pack(pady=8)
        tk.Label(header, text="Professional Edition for Pakistani University Websites",
                font=("Arial", 10), bg='#0f3460', fg='#a8d8ea').pack()
        ...
        self.status_bar = tk.Label(status_frame, text="Ready", bg='#0f3460',
                                  fg='white', anchor=tk.W, padx=10)
        self.status_bar.pack(fill=tk.X)

    def create_scan_tab(self):
        main = tk.Frame(self.scan_tab, bg='#1a1a2e', padx=30, pady=30)
        main.pack(fill=tk.BOTH, expand=True)

        tk.Label(main, text="Target URL:", font=("Arial", 14, "bold"),
                bg='#1a1a2e', fg='white').pack(anchor=tk.W, pady=(0,5))

        url_frame = tk.Frame(main, bg='#1a1a2e')
        url_frame.pack(fill=tk.X, pady=(0,20))

        self.url_entry = tk.Entry(url_frame, font=("Arial", 14), bg='#16213e',
        ...
        self.scan_btn = tk.Button(url_frame, text="START SCAN", font=("Arial", 13, "bold"),
                                  bg='#e74c3c', fg='white', padx=25, pady=10,
                                  relief=tk.FLAT, cursor='hand2', command=self.start_scan)
        self.scan_btn.pack(side=tk.RIGHT)

        # Progress bar
        prog_frame = tk.Frame(main, bg='#1a1a2e')
        prog_frame.pack(fill=tk.X, pady=20)
        self.progress_bar = ttk.Progressbar(prog_frame, mode='determinate', length=600, maximum=100)
        self.progress_bar.pack(fill=tk.X)
        self.progress_label = tk.Label(prog_frame, text="0% - Ready", bg='#1a1a2e', fg='#a8d8ea')
        self.progress_label.pack(anchor=tk.E)

        # Quick targets
        tk.Label(main, text="Quick Targets:", font=("Arial", 12, "bold"),
                bg='#1a1a2e', fg='white').pack(anchor=tk.W, pady=(20,10))

        quick_frame = tk.Frame(main, bg='#1a1a2e')
        quick_frame.pack(fill=tk.X)
        universities = [("NUST","https://nust.edu.pk"), ("SMIU","https://smiu.edu.pk"),
                        ("FAST","https://nu.edu.pk"), ("LUMS","https://lums.edu.pk"),
                        ("IBA","https://iba.edu.pk"), ("UET","https://uet.edu.pk")]
        for name, url in universities:
            tk.Button(quick_frame, text=name, bg='#0f3460', fg='white', relief=tk.FLAT,
                     cursor='hand2', padx=15, pady=5,
                     command=lambda u=url: self.set_url(u)).pack(side=tk.LEFT, padx=5)

    def create_results_tab(self):
        main = tk.Frame(self.results_tab, bg='#1a1a2e', padx=20, pady=20)
        main.pack(fill=tk.BOTH, expand=True)

        self.results_text = scrolledtext.ScrolledText(main, font=("Consolas", 10),
                                                      bg='#16213e', fg='white', wrap=tk.WORD)
        self.results_text.pack(fill=tk.BOTH, expand=True)

        # Color tags
        for tag, color in [('critical','#e74c3c'), ('high','#e67e22'), ('medium','#f1c40f'),
                          ('low','#2ecc71'), ('info','#3498db'), ('header','#a8d8ea')]:
            self.results_text.tag_configure(tag, foreground=color, font=('Consolas',10,'bold'))

        # Export buttons
        export_frame = tk.Frame(main, bg='#1a1a2e')
        export_frame.pack(fill=tk.X, pady=(10,0))
        for text, cmd, color in [("Save TXT", self.save_txt, '#27ae60'),
                                 ("Save CSV", self.save_csv, '#2980b9'),
                                 ("Save JSON", self.save_json, '#8e44ad')]:
            tk.Button(export_frame, text=text, bg=color, fg='white', relief=tk.FLAT,
                     cursor='hand2', padx=15, pady=5, command=cmd).pack(side=tk.LEFT, padx=5)

    def create_history_tab(self):
        main = tk.Frame(self.history_tab, bg='#1a1a2e', padx=20, pady=20)
        main.pack(fill=tk.BOTH, expand=True)

        columns = ('ID','Target','Date','Total','Critical','High','Medium','Low')
        self.history_tree = ttk.Treeview(main, columns=columns, show='headings', height=20)
        for col in columns:
            self.history_tree.heading(col, text=col)
            self.history_tree.column(col, width=100, anchor=tk.CENTER)
        self.history_tree.pack(fill=tk.BOTH, expand=True)

        scrollbar = ttk.Scrollbar(main, orient=tk.VERTICAL, command=self.history_tree.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.history_tree.configure(yscrollcommand=scrollbar.set)

        tk.Button(main, text="Refresh History", bg='#3498db', fg='white', relief=tk.FLAT,
                 cursor='hand2', padx=20, pady=8, command=self.load_history).pack(pady=10)
        self.load_history()

    def create_stats_tab(self):
        """Create statistics tab (initial)"""
        main = tk.Frame(self.stats_tab, bg='#1a1a2e', padx=20, pady=20)
        main.pack(fill=tk.BOTH, expand=True)

        self.stats_main = main
        self.stats_cards_frame = tk.Frame(main, bg='#1a1a2e')
        self.stats_cards_frame.pack(fill=tk.X, pady=(0, 20))

        self.refresh_stats_tab()

    def refresh_stats_tab(self):
        """Update statistics cards and chart"""
        # Clear old cards
        ...
        for title, value, color in cards:
            card = tk.Frame(self.stats_cards_frame, bg=color, height=100)
            card.pack(side=tk.LEFT, padx=10, expand=True, fill=tk.BOTH)
            card.pack_propagate(False)
            tk.Label(card, text=title, bg=color, fg='white').pack(pady=(15,5))
        ...
        if HAS_MATPLOTLIB:
            ...
            if hasattr(self, 'stats_chart_canvas'):
                self.stats_chart_canvas.get_tk_widget().destroy()

            total_issues = (stats['critical'] + stats['high'] + 
            ...
            if total_issues > 0:
                ...
                # Filter zero values to avoid pie chart error
                non_zero_labels = []
                non_zero_values = []
                non_zero_colors = []
                ...
                canvas = FigureCanvasTkAgg(fig, master=self.stats_main)
                canvas.draw()
                canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
                self.stats_chart_canvas = canvas
            else:
                # No data, show message
                tk.Label(self.stats_main,
    ...
    def update_progress(self, percent, status):
        ...
        self.root.after(0, lambda: self.status_bar.configure(text=status))
    ...
    def perform_scan(self, url):
        ...
        self.root.after(0, self.refresh_stats_tab)
